...
//		usefulMethods();
	...
	private static void usefulMethods() {
		...
		//	문자열은 char의 연속된 집합
		//	인덱스 접근, 길이를 잡을 수 있따 .
		System.out.println("str 길이 : " + str.length()); //	길이는 39 인덱스는 0~38
		System.out.println("가장 마지막 글자:" + str.charAt(str.length() - 1));
		
		//	변환 메서드
		System.out.println("모두 대문자로:" + str.toUpperCase());
		System.out.println("모두 so문자로:" + str.toLowerCase());
		...
		System.out.println("2번쨰 검색 위치:" + index);
		index = str.indexOf("Java",index +4);	//	세번쨰 검색
		System.out.println("3번째 검색 위치: "+ index);	//	-1 :찾을수 없단 의미 
		
		//	역방향 검색 : lastIndexOf()
		index = str.lastIndexOf("Java");
		System.out.println("마지막 Java위치" + index);
		...
		System.out.println(str.substring(17));	//	끝까지 추출
		
		String s2 = "		Hello, Java		";
		System.out.println(s2.trim());	//	좌우의 Whitespace 제거
		
		
		//	문자열 분리 : spilt - 구분 기호를 기준으로 문자열을 분리 -> 배열 반환
