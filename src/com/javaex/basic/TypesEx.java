...
//		explicitCastingEx();
	...
	public static void constantEx() {
		...
		final int SPEED_LIMIT = 110;	//	단어의 조합은_로 연결
	...
	public static void floatDoubleEs() {
		//	실수
		// 	float(4) < double(8)
		// 	실수 표현 범위를 넓히고 정밀로드를 일부 포기 한 것 .
		float fVar = 3.14159F;	// 뒤에 F를 명시함으로써 float라는 것을 인식 
		double dVar = 3.14159;
	...
	public static void integerTypesEx(){
		...
		int intVal; // 선언
		intVal =2020; // 할당(초기화)
//		int inVal2 = 1234567890234;
		
		long longVal1; // 8byte
		long longVal2;
		
		longVal1 = 2020;
		longVal2 = 1234567890123L;	//뒤에 L long 형 타입 큰데이터 같은 경우L 붙여줘야함 
		
		// 표기법 : 
		int bin, oct, hex;
		bin = 0b1100; // 2진수는 0b로
		oct = 072; 	  // 8진수는 0으로 시작
		hex = 0xFF;   // 16진수는 0x로 시작 
